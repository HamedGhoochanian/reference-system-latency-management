...
int main(int argc, char * argv[])
{
  ...
  RCLCPP_INFO(
    ...
    executor1.callback_priority_enabled ? "Enabled" : "Disabled");
  executor1.set_executor_priority_cpu(90, 0);
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS executor 1's rt-priority %d and CPU %d", executor1.executor_priority,
    executor1.executor_cpu);

  executor2.enable_callback_priority();
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS priority-based callback scheduling: %s",
    executor2.callback_priority_enabled ? "Enabled" : "Disabled");
  executor2.set_executor_priority_cpu(89, 1);
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS executor 2's rt-priority %d and CPU %d", executor2.executor_priority,
    executor2.executor_cpu);

  executor3.enable_callback_priority();
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS priority-based callback scheduling: %s",
    executor3.callback_priority_enabled ? "Enabled" : "Disabled");
  executor3.set_executor_priority_cpu(88, 2);
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS executor 3's rt-priority %d and CPU %d", executor3.executor_priority,
    executor3.executor_cpu);

  executor4.enable_callback_priority();
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS priority-based callback scheduling: %s",
    executor4.callback_priority_enabled ? "Enabled" : "Disabled");
  executor4.set_executor_priority_cpu(87, 3);
  RCLCPP_INFO(
    rclcpp::get_logger(
      "rclcpp"), "PiCAS executor 4's rt-priority %d and CPU %d", executor4.executor_priority,
    executor4.executor_cpu);

  std::vector<std::string> executor1_nodes {"VehicleDBWSystem", "VehicleInterface", "MPCController",
    "BehaviorPlanner"};
  std::vector<std::string> executor2_nodes {"FrontLidarDriver", "RearLidarDriver",
    "PointsTransformerFront",
    "PointsTransformerRear", "PointCloudFusion", "RayGroundFilter", "ObjectCollisionEstimator"};
  std::vector<std::string> executor3_nodes {"EuclideanClusterDetector", "EuclideanClusterSettings",
    "IntersectionOutput"};
  std::vector<std::string> executor4_nodes {"PointCloudMap", "PointCloudMapLoader",
    "VoxelGridDownsampler", "NDTLocalizer",
    "Visualizer", "Lanelet2Map", "LanePlanner", "ParkingPlanner", "Lanelet2MapLoader",
