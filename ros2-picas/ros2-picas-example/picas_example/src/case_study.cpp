...
public:
  StartNode(
    const std::string node_name, const std::string pub_topic,
    std::shared_ptr<trace::Trace> trace_ptr, int exe_time, int period, bool end_flag)
  : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(USE_INTRA_PROCESS_COMMS)), count_(
      0), trace_callbacks_(trace_ptr), exe_time_(exe_time), period_(period), end_flag_(end_flag)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "Create wall timer at %ld",
...
public:
  IntermediateNode(
    const std::string node_name, const std::string sub_topic,
    const std::string pub_topic, std::shared_ptr<trace::Trace> trace_ptr,
    int exe_time, bool end_flag)
  : Node(node_name, rclcpp::NodeOptions().use_intra_process_comms(USE_INTRA_PROCESS_COMMS)), count_(
      0), trace_callbacks_(trace_ptr), exe_time_(exe_time), end_flag_(end_flag)
  {
    subscription_ = this->create_subscription<test_interfaces::msg::TestString>(
      sub_topic, 1, std::bind(
        &IntermediateNode::callback, this,
        _1));
    if (pub_topic != "") {
      publisher_ = this->create_publisher<test_interfaces::msg::TestString>(pub_topic, 1);
    }
  ...
  void callback(const test_interfaces::msg::TestString::SharedPtr msg)
  {
    ...
    if (end_flag_) {
      ...
      trace_callbacks_->trace_write_count(
        name + "_latency",
        std::to_string(latency_time.tv_sec * 1000000 + latency_time.tv_usec), message.data);
    }
...
int main(int argc, char * argv[])
{
  ...
  while (1) {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger(
        "rclcpp"), "dummy_load_calib: %d (duration_us: %d ns)", dummy_load_calib, duration_us);
  ...
  auto task2 = std::make_shared<IntermediateNode>(
    "C1R1_12_30", "task1", "task2", trace_callbacks,
    16, true);

  auto task3 = std::make_shared<IntermediateNode>(
    "C2R1_11_27", "task1", "task3", trace_callbacks,
    2, false);
  auto task4 = std::make_shared<IntermediateNode>(
    "C2R2_11_28", "task3", "task4", trace_callbacks,
    18, false);
  auto task5 = std::make_shared<IntermediateNode>(
    "C2R3_11_29", "task4", "task5", trace_callbacks,
    9, true);

  auto task6 = std::make_shared<StartNode>("C3T_10_22", "task6", trace_callbacks, 23, 100, false);
  auto task7 = std::make_shared<IntermediateNode>(
    "C3R1_10_23", "task6", "task7", trace_callbacks,
    8, false);
  auto task8 = std::make_shared<IntermediateNode>(
    "C3R2_10_24", "task7", "task8", trace_callbacks,
    14, false);
  auto task9 = std::make_shared<IntermediateNode>(
    "C3R3_10_25", "task8", "task9", trace_callbacks,
    18, true);

  auto task10 = std::make_shared<StartNode>("C4T_9_19", "task10", trace_callbacks, 21, 100, false);
  auto task11 = std::make_shared<IntermediateNode>(
    "C4R1_9_20", "task10", "task11", trace_callbacks,
    18, false);
  auto task12 = std::make_shared<IntermediateNode>(
    "C4R2_9_21", "task11", "task12", trace_callbacks,
    7, true);

  auto task13 = std::make_shared<StartNode>("C5T_8_15", "task13", trace_callbacks, 2, 160, false);
  auto task14 = std::make_shared<IntermediateNode>(
    "C5R1_8_16", "task13", "task14", trace_callbacks,
    11, false);
  auto task15 = std::make_shared<IntermediateNode>(
    "C5R2_8_17", "task14", "task15", trace_callbacks,
    7, false);
  auto task16 = std::make_shared<IntermediateNode>(
    "C5R3_8_18", "task15", "task16", trace_callbacks,
    8, true);

  auto task17 = std::make_shared<StartNode>("C6T_7_13", "task17", trace_callbacks, 2, 1000, false);
  auto task18 = std::make_shared<IntermediateNode>(
    "C6R1_7_14", "task17", "task18", trace_callbacks,
    196, true);

  auto task19 = std::make_shared<StartNode>("C7T_6_11", "task19", trace_callbacks, 33, 120, false);
  auto task20 = std::make_shared<IntermediateNode>(
    "C7R1_6_12", "task19", "task20", trace_callbacks,
    2, true);

  auto task21 = std::make_shared<StartNode>("C8T_5_9", "task21", trace_callbacks, 33, 120, false);
  auto task22 = std::make_shared<IntermediateNode>(
    "C8R1_5_10", "task21", "task22", trace_callbacks,
    7, true);

  auto task23 = std::make_shared<StartNode>("C9T_4_7", "task23", trace_callbacks, 33, 120, false);
  auto task24 = std::make_shared<IntermediateNode>(
    "C9R1_4_8", "task23", "task24", trace_callbacks,
    7, true);

  auto task25 = std::make_shared<StartNode>("C10T_3_5", "task25", trace_callbacks, 33, 120, false);
  auto task26 = std::make_shared<IntermediateNode>(
    "C10R1_3_6", "task25", "task26", trace_callbacks,
    2, true);

  auto task27 = std::make_shared<StartNode>("C11T_2_3", "task27", trace_callbacks, 33, 120, false);
  auto task28 = std::make_shared<IntermediateNode>(
    "C11R1_2_4", "task27", "task28", trace_callbacks,
    2, true);

  auto task29 = std::make_shared<StartNode>("C12T_1_1", "task29", trace_callbacks, 33, 120, false);
  auto task30 = std::make_shared<IntermediateNode>(
    "C12R1_1_2", "task29", "task30", trace_callbacks,
    2, true);
  ...
  exec3.remove_node(task6); exec3.remove_node(task7); exec3.remove_node(task8); exec3.remove_node(
    task9);
  exec4.remove_node(task10); exec4.remove_node(task11); exec4.remove_node(task12);
  exec5.remove_node(task13); exec5.remove_node(task14); exec5.remove_node(task15);
  exec5.remove_node(task16);
  exec6.remove_node(task17); exec6.remove_node(task18);

  exec7.remove_node(task19); exec8.remove_node(task20); exec9.remove_node(task21);
  exec10.remove_node(task22);
  exec11.remove_node(task23); exec12.remove_node(task24); exec13.remove_node(task25);
  exec14.remove_node(task26);
  exec15.remove_node(task27); exec16.remove_node(task28); exec17.remove_node(task29);
  exec18.remove_node(task30);
#else
  // Allocate callbacks to executors
  exec1.add_node(task1); exec1.add_node(task2);
  ...
  exec1.remove_node(task1); exec1.remove_node(task2);
  exec1.remove_node(task13); exec1.remove_node(task14); exec1.remove_node(task15);
  exec1.remove_node(task16);
  exec1.remove_node(task23); exec1.remove_node(task26); exec1.remove_node(task27);

  exec2.remove_node(task3); exec2.remove_node(task4); exec2.remove_node(task5);
  exec2.remove_node(task21); exec2.remove_node(task20); exec2.remove_node(task28);
  exec2.remove_node(task29);
